...
class ElectrodeMesh():
    ...
    def __init__(self, density=30, lb=(0, 0), ru=(0.7, 0.7)) -> None:
        ...
        for facet in facets(initial_mesh):
            if facet.midpoint().distance(Point(lb_electrode[0], lb_electrode[1])) < 0.01:
                boundary_markers[facet] = 1  # Mark region near electrode
            if facet.midpoint().distance(Point(ru_electrode[0], ru_electrode[1])) < 0.01:
               boundary_markers[facet] = 1  # Mark region near electrode
            if facet.midpoint().distance(Point(lb_electrode[0], ru_electrode[1])) < 0.01:
               boundary_markers[facet] = 1  # Mark region near electrode
            if facet.midpoint().distance(Point(ru_electrode[0], lb_electrode[1])) < 0.01:
                boundary_markers[facet] = 1  # Mark region near electrode
                
            if facet.midpoint().distance(Point(0.5, 0.5)) < 0.02:
                boundary_markers[facet] = 1  # Mark region near electrode
