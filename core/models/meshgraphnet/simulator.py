...
class Simulator(nn.Module):
    ...
    def forward(self, graph:Data, **argv):
        ...
        #temp_noise = torch.normal(mean=0.05, std=0.02, size=(graph.x.shape[0], 1), device=graph.x.device)  # Noise ~ N(0, 1)
        #volt_noise = torch.normal(mean=0.02, std=0.01, size=(graph.x.shape[0], 1), device=graph.x.device)  # Noise ~ N(0, 0.5)
