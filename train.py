...
delta_t = 0.5 # Mess around with this
...
model = msgPassing(message_passing_num=7, node_input_size=3+out_ndim, edge_input_size=3, 
                   ndim=out_ndim, device=device, model_dir=ckptpath)    # Mess with MPN# to 2 or 3
model.to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-5)

mesh = ElectrodeMesh(ru=(0.75, 0.75), lb=(0.25, 0.25), density=35)
...
setattr(train_config, 'train_steps', 30)    # 1 minute total simulation
setattr(train_config, 'epchoes', 700)
