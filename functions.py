...
class ElectroThermalFunc(): 
    ...
    def init_condition(self, pos):
        ...
        temp = torch.full_like(pos[:, 0:1], 37)  # Create a tensor filled with 310s for the temperature
        volt = torch.full_like(pos[:, 1:2], 0.5)    # Create a tensor filled with 1s for the voltage
    ...
    def boundary_condition(self, pos):
        ...
        temp = torch.full_like(pos[:, 0:1], 37)  # Create a tensor filled with 310s for the temperature
        volt = torch.full_like(pos[:, 1:2], 0.2)    # Create a tensor filled with 1s for the voltage
